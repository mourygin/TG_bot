...
async def get_buying_list(message):
    ...
    for i in range(0,items):
        if i == items-1:
            with open(pics[i - 1], 'rb') as img:
                await message.answer_photo(img,vine_names[i] + '\n' + texts[i] + f'\nЦена - {price[i]} руб.', reply_markup=kb_vine)
        else:
            with open(pics[i], 'rb') as img:
                await message.answer_photo(img,vine_names[i] + '\n' + texts[i] + f'\nЦена - {price[i]} руб.')
